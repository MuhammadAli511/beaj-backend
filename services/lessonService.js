...
const deleteLessonService = async (id) => {
    try {
        ...
        if (activity == 'listenAndSpeak' || activity == 'watchAndSpeak' || activity == 'conversationalQuestionsBot' || activity == 'conversationalMonologueBot' || activity == 'conversationalAgencyBot') {
...
const migrateLessonService = async (lessonId, courseId) => {
    ...
    try {
        ...
        try {
            ...
            if (['listenAndSpeak', 'watchAndSpeak', 'conversationalQuestionsBot', 'conversationalMonologueBot', 'conversationalAgencyBot'].includes(lesson.activity)) {
                const speakActivityQuestionFiles = await speakActivityQuestionRepository.getByLessonId(lesson.LessonId);
            ...
            } else if (['mcqs'].includes(lesson.activity)) {
                const multipleChoiceQuestions = await multipleChoiceQuestionRepository.getByLessonId(lesson.LessonId);
