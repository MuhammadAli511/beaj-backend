...
const getActivtyWiseWeeklyScore_max = async (course_id,grp) => {
    try {
        // console.log("Parameters:", { date, course_id, grp, cohort });
        const qry = `
           WITH target_group_users AS (
...
SELECT 
     ...
     m."name",
	  ...
	  max(round(COALESCE(ls.listenAndSpeak_week1_total, 0), 2) + round(COALESCE(ls.listenAndSpeak_week2_total, 0), 2)
		 	+ round(COALESCE(ls.listenAndSpeak_week3_total, 0), 2) + round(COALESCE(ls.listenAndSpeak_week4_total, 0), 2))
	  as "listenAndSpeak_total",
	 
	 

      max(round(COALESCE(mc.mcqs_week1_total, 0), 2) + round(COALESCE(mc.mcqs_week2_total, 0), 2)
		 	+ round(COALESCE(mc.mcqs_week3_total, 0), 2) + round(COALESCE(mc.mcqs_week4_total, 0), 2))
	  as "mcqs_total",
      ...
      max(round(COALESCE(ws.watchAndSpeak_week1_total, 0), 2) + round(COALESCE(ws.watchAndSpeak_week2_total, 0), 2)
		 	+ round(COALESCE(ws.watchAndSpeak_week3_total, 0), 2) + round(COALESCE(ws.watchAndSpeak_week4_total, 0), 2))
	  as "watchAndSpeak_total",
      ...
      max(round(COALESCE(rd.read_week1_total, 0), 2) + round(COALESCE(rd.read_week2_total, 0), 2) + round(COALESCE(rd.read_week3_total, 0), 2)
		 	+ round(COALESCE(rd.read_week4_total, 0), 2))
	  as "read_total",
      ...
      max(round(COALESCE(cm.conversationalMonologue_week1_total, 0), 2) + round(COALESCE(cm.conversationalMonologue_week2_total, 0), 2) +
		 	round(COALESCE(cm.conversationalMonologue_week3_total, 0), 2) + round(COALESCE(cm.conversationalMonologue_week4_total, 0), 2))
	  as "conversationalMonologue_total",


	 
     max(round(COALESCE(sp.Speaking_practice_week1_total, 0), 2) + round(COALESCE(sp.Speaking_practice_week2_total, 0), 2) + 
		 	round(COALESCE(sp.Speaking_practice_week3_total, 0), 2) + round(COALESCE(sp.Speaking_practice_week4_total, 0), 2))
	  as "Speaking_practice_total",
...
export default { getDataFromPostgres, getDataActivityComplete, getWeeklyActivityCompleted,getUserMetadataAll,getUserMetadataTime,getLessonCompletions,getActivity_Completions, getWeeklyScore, getActivtyWiseWeeklyScore_max,
