...
const validation = async (activities) => {
    ...
    try {
        for (const activity of activities) {
            ...
            // For audio activity questionAudio should exist (This is only extra validation for audio activity)
            if (!activity.questions?.some(q => q.questionAudio)) {
                allErrors.push(`audio activity from "${activity.startRow}" to "${activity.endRow}" should have question audio`);
            }
        }
        ...
        return {
            errors: [`audio validation error: ${error.message}`],
            toCreateCount: 0,
            toUpdateCount: 0,
        };
    }
...
const ingestion = async (activities) => {
    ...
    try {
        ...
        return {
            errors: [`audio ingestion error: ${error.message}`],
            createdCount: 0,
            updatedCount: 0
        };
    }
