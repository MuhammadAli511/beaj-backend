...
const validation = async (activities) => {
    ...
    try {
        for (const activity of activities) {
            ...
            // For listenAndSpeak activity questionText should exist
            if (!activity.questions?.some(q => q.questionText)) {
                allErrors.push(`listenAndSpeak activity from "${activity.startRow}" to "${activity.endRow}" should have question text`);
            }

            // For listenAndSpeak activity questionVideo or questionAudio should exist (at least one)
            if (!activity.questions?.some(q => q.questionVideo || q.questionAudio)) {
                allErrors.push(`listenAndSpeak activity from "${activity.startRow}" to "${activity.endRow}" should have question video`);
            }

            // For each question atleast one answerText should exist
            if (!activity.questions?.some(q => q.answers?.some(a => a.answerText))) {
                allErrors.push(`listenAndSpeak activity from "${activity.startRow}" to "${activity.endRow}" should have atleast one answer text`);
            }
        }
        ...
        return {
            errors: [`listenAndSpeak validation error: ${error.message}`],
            toCreateCount: 0,
            toUpdateCount: 0,
        };
    }
...
const ingestion = async (activities) => {
    ...
    try {
            ...
            try {
                ...
                if (activity.questions && activity.questions.length > 0) {
                    for (const question of activity.questions) {
                        if (question.questionVideo || question.questionAudio && question.questionText) {
                            ...
                            try {
                                ...
                                if (question.questionAudio) {
                                    ...
                                    audioFile = await getDriveMediaUrl(question.questionAudio);
        ...
        return {
            errors: [`listenAndSpeak ingestion error: ${error.message}`],
            createdCount: 0,
            updatedCount: 0
        };
    }
