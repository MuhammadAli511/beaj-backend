...
const validation = async (activities) => {
    ...
    try {
        for (const activity of activities) {
            ...
            // For read activity questionVideo should exist (This is only extra validation for read activity)
            if (!activity.questions?.some(q => q.questionVideo)) {
                allErrors.push(`read activity from "${activity.startRow}" to "${activity.endRow}" should have question video`);
            }
        }
        ...
        return {
            errors: [`read validation error: ${error.message}`],
            toCreateCount: 0,
            toUpdateCount: 0,
        };
    }
