...
const validation = async (activities) => {
    ...
    try {
        for (const activity of activities) {
            ...
            // For imageEnd activity questionImage should exist (This is only extra validation for imageEnd activity)
            if (!activity.questions?.some(q => q.questionImage)) {
                allErrors.push(`imageEnd activity from "${activity.startRow}" to "${activity.endRow}" should have question image`);
            }
        }
        ...
        return {
            errors: [`imageEnd validation error: ${error.message}`],
            toCreateCount: 0,
            toUpdateCount: 0,
        };
    }
...
const ingestion = async (activities) => {
    ...
    try {
        ...
        return {
            errors: [`imageEnd ingestion error: ${error.message}`],
            createdCount: 0,
            updatedCount: 0
        };
    }
