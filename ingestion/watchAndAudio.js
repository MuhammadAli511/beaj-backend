...
const validation = async (activities) => {
    ...
    try {
        for (const activity of activities) {
            ...
            // For watchAndAudio activity questionVideo or questionAudio should exist (at least one)
            if (!activity.questions?.some(q => q.questionVideo || q.questionAudio)) {
                allErrors.push(`watchAndAudio activity from "${activity.startRow}" to "${activity.endRow}" should have question video or question audio`);
            }
        }
        ...
        return {
            errors: [`watchAndAudio validation error: ${error.message}`],
            toCreateCount: 0,
            toUpdateCount: 0,
        };
    }
...
const ingestion = async (activities) => {
    ...
    try {
        ...
        return {
            errors: [`watchAndAudio ingestion error: ${error.message}`],
            createdCount: 0,
            updatedCount: 0
        };
    }
