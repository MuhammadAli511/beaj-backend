...
const validation = async (activities) => {
    ...
    try {
        for (const activity of activities) {
            ...
            // For feedbackAudio activity questionText should exist
            if (!activity.questions?.some(q => q.questionText)) {
                allErrors.push(`feedbackAudio activity from "${activity.startRow}" to "${activity.endRow}" should have question text`);
            }

            // For feedbackAudio activity questionAudio should exist
            if (!activity.questions?.some(q => q.questionAudio)) {
                allErrors.push(`feedbackAudio activity from "${activity.startRow}" to "${activity.endRow}" should have question audio`);
            }
        }
        ...
        return {
            errors: [`feedbackAudio validation error: ${error.message}`],
            toCreateCount: 0,
            toUpdateCount: 0,
        };
    }
